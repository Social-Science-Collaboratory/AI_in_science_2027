...
draw_mock_watermark <- function(x, y, width, height,
  ...
  # still fits if the panel is regenerated at another aspect. "MOCK" in bold is
  # about 2.9 em wide and 0.72 em tall; rotating that box by `angle` spreads it
  # over both axes, and the binding constraint is whichever axis runs out
  # first. Solving only for width (the obvious version) overflows the short
  # axis and clips the M and K against the panel edge.
  a <- angle * pi / 180
  em_w <- 2.9
  em_h <- 0.72
